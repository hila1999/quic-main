...
async def send_data(stream_socket, file_path, port, bytes_counters, total_bytes_counter, packets_counters, total_packets_counter):
    ...
    with open(file_path, 'rb') as file:
        while True:
            # Dynamically adjust the chunk size to stay within the desired range of bits (1000 - 2000)
            chunk_size = max(1000, min(2000, 1024))  # Ensure chunk size is within the desired range
